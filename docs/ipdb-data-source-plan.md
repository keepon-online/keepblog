...
- 冷启动缺失下载（自管 Release 优先，官方源兜底）；
- 每日 03:00 运行时更新（仅自管 Release，SHA-256 校验，不跟踪上游 master）；
- 下载失败保留旧文件与查询器；
- xdb 加载校验和查询器热切换；
- 独立 CI 构建、校验并发布 Release（`ipdb-latest` 滚动 + `ipdb-v*` 归档），运行时更新闭环见 [ipdb.md](./ipdb.md)。
...
> 状态：**进行中**。采集已上线：`pkg/area/stats.go` 在 `Area()` 入口做纯内存原子计数（查询总数、命中/未命中/无库、内网地址、云 CDN 出口、国家·省份·城市·ISP 字段空置率），每日 03:10 定时任务输出 JSON 快照到日志（`ipdb_stats` 前缀）并清零，日志即历史记录。不保存任何原始 IP。IPv6 流量当前不经过 IPv4 查询路径，该口径暂为 0。积累 1~2 周后从 `logs/app.log` 提取基线，再决定是否进入阶段二。
