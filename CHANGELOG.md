...
### 修复
- **仪表盘三图表首屏空白（流量趋势/分类饼图/访问频次柱状图）**：3.2.0 图表组件重构引入渲染竞态——`v-if` 控制容器渲染，immediate watch 在 setup 阶段触发时 ref 尚为 null，`updateChart` 的判空提前返回吞掉唯一一次 `setOptions` 调用，而 `useECharts` 仅在 `setOptions` 内懒初始化，此后再无触发时机，图表永远空白。去除三处判空提前返回（`setOptions` 内部等元素就绪后自动补建实例）。
